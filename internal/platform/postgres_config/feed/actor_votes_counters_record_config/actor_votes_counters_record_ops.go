...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) UpsertActorVotesCountersRecord (
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) DeleteActorVotesCountersRecordsByPostHash(postHash string) {
  _, err := actorVotesCountersRecordExecutor.C.Exec(DELETE_ACTOR_VOTES_COUNTERS_RECORDS_BY_POST_HASH_COMMAND, postHash)
  if err != nil {
    log.Panicf("Failed to delete actor votes counters records for postHash %s with error: %+v\n", postHash, err)
  }
  log.Printf("Sucessfully deleted actor votes counters records for postHash %s\n", postHash)
}

func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) DeleteActorVotesCountersRecordsByActor(actor string) {
  _, err := actorVotesCountersRecordExecutor.C.Exec(DELETE_ACTOR_VOTES_COUNTERS_RECORDS_BY_ACTOR_COMMAND, actor)
  if err != nil {
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) DeleteActorVotesCountersRecordsByPostHashAndActor(
  postHash string, actor string) {
  _, err := actorVotesCountersRecordExecutor.C.Exec(DELETE_ACTOR_VOTES_COUNTERS_RECORDS_BY_POST_HASH_AND_ACTOR_COMMAND,
    postHash, actor)
  if err != nil {
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) GetTotalReputationByPostHash(
  postHash string) feed_attributes.Reputation {
  var totalReputation sql.NullInt64
  err := actorVotesCountersRecordExecutor.C.Get(&totalReputation, QUERY_TOTAL_REPUTATION_BY_POST_HASH_COMMAND, postHash)
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) GetReputationByPostHashAndActor(
    postHash string, actor string) feed_attributes.Reputation {
  var reputation sql.NullInt64
  err := actorVotesCountersRecordExecutor.C.Get(&reputation, QUERY_REPUTATION_BY_POST_HASH_AND_ACTOR_COMMAND,
    postHash, actor)
  if err != nil && err != sql.ErrNoRows {
    log.Panicf(
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) GetReputationByPostHashAndVoteType(
    postHash string, voteType feed_attributes.VoteType) feed_attributes.Reputation {
  var reputation sql.NullInt64
  err := actorVotesCountersRecordExecutor.C.Get(&reputation, QUERY_REPUTATION_BY_POST_HASH_AND_VOTE_TYPE_COMMAND,
    postHash, voteType)
  if err != nil && err != sql.ErrNoRows {
    log.Panicf(
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) GetReputationByPostHashAndActorWithLatestVoteTypeAndTimeCutOff(
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) GetTotalVotesCountByPostHashAndActorType(
    postHash string, actor string) int64 {
  var postVotes sql.NullInt64
  err := actorVotesCountersRecordExecutor.C.Get(&postVotes, QUERY_TOTAL_VOTES_COUNT_BY_POST_HASH_AND_ACTOR_COMMAND, postHash, actor)
  if err != nil && err != sql.ErrNoRows {
    log.Panicf(
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) GetActorListByPostHashAndVoteType(
    postHash string, voteType feed_attributes.VoteType) *[]string {
  var actorList []string
  err := actorVotesCountersRecordExecutor.C.Select(&actorList, QUERY_ACTOR_LIST_BY_POST_HASH_AND_VOTE_TYPE_COMMAND, postHash, voteType)
  if err != nil && err != sql.ErrNoRows {
    log.Panicf(
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) GetActorVotesCountersRecordByPostHashAndActor (
    postHash string, actor string) *ActorVotesCountersRecord {
  var postReputationsRecord ActorVotesCountersRecord
  err := actorVotesCountersRecordExecutor.C.Get(
...
func (actorVotesCountersRecordExecutor *ActorVotesCountersRecordExecutor) GetTotalActorReputationByPostHash(postHash string) int64 {
  var totalReputations int64
  err := actorVotesCountersRecordExecutor.C.Get(&totalReputations, QUARY_TOTAL_REPUTATION_BY_POSTHASH_COMMAND,postHash)
  if err != nil {
