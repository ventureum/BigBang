...
func TestHandler(t *testing.T) {
  ...
  for _, test := range tests {
    responseMessageProfile := api.SendPost(test.request, api.ProfileAlphaEndingPoint)
    var responseProfile lambda_profile_config.Response
    mapstructure.Decode(*responseMessageProfile, &responseProfile)
    assert.Equal(t, test.response, responseProfile)
  }
}
